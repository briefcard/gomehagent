...
- **`slug`**(phrase) → `str`  ·  from `planner.py`, `skill_pack.py`, `web.py`
...
- **`targets`**(tenant, tier?, status?, cluster_key?) → `list[db.KeywordTarget]`  ·  from `funnel.py`, `planner.py`, `skill_pack.py`, `test_blog_skill.py`, `test_keyword_progress.py`, `test_keywords.py`, `test_link_flag.py`, `test_pointer_fixes.py`, `test_support_links.py`
...
- **`article_window`**(sysrow) → `dict`  ·  from `web.py`
...
- **`next_article_slot`**(win, slot) → `—`  ·  from `web.py`
- **`rest_days_for`**(sysrow) → `int`  ·  from `test_moment_pressure.py`
- **`took_slot`**(win, slot) → `dt.date`  ·  from `web.py`
...
- **`plans`**(tenant, key?, due_by?) → `list[db.SystemRun]`  ·  from `admin_ui.py`, `planner.py`, `test_plan_lifecycle.py`, `test_planner.py`, `test_plans.py`, `test_schedule_nav.py`, `test_support_links.py`, `test_workflow_ui.py`, `worker.py`
