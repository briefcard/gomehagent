...
- **`board_inputs`**(tenant, entity_key, product_id?, boards?) → `dict`  ·  from `test_a_pinterest_board_link_fills_a_board.py`
...
- **`with_references`**(prompt, product?, look?, shape?, n?, with_people?, model?, checklist?) → `dict`  ·  from `creative.py`
