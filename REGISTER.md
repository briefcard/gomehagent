...
- **`eligible`**(tenant, intent?, fmt?, recent_shapes?, recent_designs?) → `list[dict]`  ·  **from nothing**
...
- **`library`**(review?) → `list[dict]`  ·  from `admin_ui.py`, `recreate_once.py`, `test_the_model_makes_the_email.py`
...
- **`swipes`**(tenant?) → `list[dict]`  ·  from `admin_ui.py`
