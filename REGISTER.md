...
- **`file_reference`**(asset_id, brief?, source_url?) → `dict`  ·  from `recreate.py`
...
- **`latest`**(tenant) → `dict`  ·  from `admin_ui.py`, `test_the_model_makes_the_email.py`
...
- **`latest`**(structure_id, tenant) → `dict | None`  ·  from `admin_ui.py`, `test_the_model_makes_the_email.py`
