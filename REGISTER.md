...
- **`batch`**(tenant, commitment?, positioning?, entity_key?, audience_key?, claim?, prominent?, headline?, subline?, fmt?, output_id?, situation?, plates?, review?, boards?, image_model?) → `dict`  ·  from `bakeoff.py`, `test_a_kept_frame_has_its_placements_and_no_painted_type.py`, `test_a_pinterest_board_link_fills_a_board.py`, `test_a_run_that_made_nothing_says_why.py`, `test_a_second_image_provider_is_a_call_away.py`, `test_an_almost_product_is_not_the_product.py`, `test_creative_batch.py`, `test_the_board_reaches_the_generator.py`, `test_the_product_is_judged_against_its_photographs.py`
- **`batch_each`**(tenant, models?, **kw) → `dict`  ·  from `test_the_image_model_is_chosen_where_the_set_starts.py`
...
- **`edit`**(prompt, images?, model?, shape?, n?, tenant?) → `dict`  ·  from `imagegen.py`
