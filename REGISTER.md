...
- **`check_page`**(tenant, url, html?) → `dict`  ·  from `test_the_ban_list_reaches_the_fields.py`
...
- **`board_inputs`**(tenant, entity_key, product_id?, boards?) → `dict`  ·  from `test_a_pinterest_board_link_fills_a_board.py`, `test_an_almost_product_is_not_the_product.py`
...
- **`may_publish`**(asset_id) → `tuple[bool, str]`  ·  from `approvals.py`, `compose.py`, `creative.py`, `ledger.py`, `test_assets.py`, `test_every_system_draws.py`, `test_strategy_ledger.py`, `web.py`
...
- **`review_asset`**(asset_id, approve, by?, rights?) → `str`  ·  from `test_a_canva_import_that_fails_falls_back_to_the_picture.py`, `test_a_frame_arrives_in_canva_as_layers.py`, `test_a_kept_frame_has_its_placements_and_no_painted_type.py`, `test_assets.py`, `test_creative_batch.py`, `test_creative_seam.py`, `test_hosting.py`, `test_strategy_ledger.py`, `web.py`
