...
- **`render_content`**(key, tenant?, started?, err?, msg?, cpage?, sub?, q?, flt?, corigin?) → `str`  ·  from `test_brand_sources.py`, `test_canva_falls_back_to_the_agency.py`, `test_creative_batch.py`, `test_draft_sync.py`, `test_kb_ui.py`, `test_provenance.py`, `test_review_tab.py`, `test_ship_section.py`, `test_the_board_reaches_the_generator.py`, `web.py`
...
- **`batch`**(tenant, commitment?, positioning?, entity_key?, audience_key?, claim?, prominent?, headline?, subline?, fmt?, output_id?, situation?, plates?, review?, boards?) → `dict`  ·  from `test_creative_batch.py`, `test_the_board_reaches_the_generator.py`
...
- **`input_image`**(blob, max_side?) → `tuple[bytes, str]`  ·  from `creative.py`
...
- **`with_references`**(prompt, product?, look?, shape?, n?, with_people?, model?) → `dict`  ·  from `creative.py`
...
- **`add_board`**(tenant, name, note?) → `str`  ·  from `test_ad_board.py`, `test_the_board_reaches_the_generator.py`, `web.py`
...
- **`set_board_role`**(asset_id, board_slug, role, on?) → `str`  ·  from `test_the_board_reaches_the_generator.py`, `web.py`
