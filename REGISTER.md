...
- **`assemble`**(tenant, text, sender?, model_fn?) → `Brief`  ·  from `brief_demo.py`, `portal_ui.py`, `test_client_report.py`, `test_metrics.py`, `test_selection.py`, `web.py`
...
- **`effectiveness`**() → `list[dict]`  ·  from `gen_systems_reference.py`, `test_effectiveness.py`, `test_lead_reply.py`
