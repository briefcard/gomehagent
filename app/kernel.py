...
def _dispatch(role: Role, name: str, args: dict, session_files: dict,
              tenant: str) -> str:
    """Route one tool call for this role.

    The boundary, the dispatch and the ledger entry live in `app/tools.py` now,
    because they were written out here and NOWHERE ELSE — and `triage` runs a
    second model loop that therefore applied the boundary and filed nothing.
    Three steps kept inside one caller are three steps the next loop will do two
    of. All this adds is the role's own handler and its session files.
    """
    from . import tools
    return tools.call(
        name, args, tenant,
        fallback=lambda n, a: role.dispatch(n, a, session_files),
        source=role.name or "kernel")
