...
def health(key: str = Depends(admin_key)) -> dict:
    ...
    base.update({"whatsapp": config.WHATSAPP_ENABLED,
                 ...
                 "oauth": _oauth_setup(),
                 # WHICH PIPELINES CAN ACTUALLY RUN. The owner had to ask what
                 # was connected, and nothing anywhere could answer him:
                 # per-client connections live on the Accounts tab, but the
                 # service-level API keys are environment variables that
                 # appear on no surface at all. Creative needs TWO of them,
                 # from two different companies, and the failure when one is
...
def _capability_report() -> dict:
    """Which service-level keys are SET, and what each one lets the system do.

    PRESENCE ONLY. Never a value, never a prefix, never a length — the
    question is "can this run", and anything more is a credential leak wearing
    a diagnostic's clothes. Keyed like the rest of this block, because naming
    the infrastructure is exactly what the unauthenticated half refuses to do.

    Derived from what the code actually reads, so a provider added later is a
    row here rather than a thing somebody remembers to mention.
    """
    have = {
    ...
    return {
        "keys_present": have,
        # Named as the JOB rather than the key, because "is OPENAI_API_KEY
        # set" is not the question anybody has. "Can we make a picture" is.
        "can": {
            "draft_copy": have["anthropic"],
            "generate_images": have["openai"],
            "review_generated_images": have["anthropic"],
            "semantic_search": have["openai"],
        },
        # The two-provider fact, said once, where somebody looking at a broken
        # pipeline will see it.
        "note": ("creative needs BOTH: images are generated by OpenAI "
                 "(gpt-image-1) and reviewed by Anthropic "
                 f"({config.CREATIVE_REVIEW_MODEL}). Per-client connections "
                 f"— Shopify, an ESP, Meta Ads — are separate and live on the "
                 f"Accounts tab."),
    }
...
async def brand_update(request: Request, key: str = Depends(admin_key)):
    ...
    from_model = form.get("image_model") is not None
    if from_model:
...
async def ad_frames(request: Request, key: str = Depends(admin_key)):
    ...
    from . import imagegen as _ig, kb as _kbd
...
async def article_picture(request: Request, key: str = Depends(admin_key)):
    ...
    # THE MODEL: the form's choice, starting from the brand's default. One
    # picture is one model — "both" is a per-set choice on the make-frames
    # form and is refused here rather than drawn twice.
    from . import imagegen as _ig
    models, why = _ig.chosen(str(form.get("image_model", "") or ""),
                             default=kbm.image_model(tenant))
    if not models:
        return _back(why)
    if len(models) > 1:
