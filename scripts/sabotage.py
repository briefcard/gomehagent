...
SABOTAGES = [
    ...
    },
    {
        ...
        'file': 'app/creative.py',
        'find': '    best_v, best_b = max(verdicts, key=lambda vb: (0 if vb[0].get("lettering") else 1,\n                                                    0 if vb[0].get("other_products") else 1,\n                                                    vb[0].get("match", 0)))',
        'replace': '    best_v, best_b = max(verdicts, key=lambda vb: (1, 0 if vb[0].get("other_products") else 1, vb[0].get("match", 0)))  # SABOTAGE',
        'suites': ['test_a_kept_frame_has_its_placements_and_no_painted_type.py'],
    ...
    },
    {
        ...
        'file': 'app/creative.py',
        'find': '                while ((wrong or best_v.get("lettering") or best_v.get("other_products"))\n                       and tries < REDRAFTS):',
        'replace': '                while ((wrong or best_v.get("other_products"))  # SABOTAGE\n                       and tries < REDRAFTS):',
        'suites': ['test_a_kept_frame_has_its_placements_and_no_painted_type.py'],
    ...
    },
    {
        ...
        'file': 'app/creative.py',
        'find': '                if wrong or best_v.get("other_products"):\n                    fidelity["not_the_product"] += 1',
        'replace': '                if False:  # SABOTAGE\n                    fidelity["not_the_product"] += 1',
    ...
    {
        'name': 'the_scene_holds_no_other_product',
        'file': 'app/imagegen.py',
        'find': '            rules.append(_ONLY_THIS_PRODUCT)\n',
        'replace': '            pass  # SABOTAGE\n',
        'suites': ['test_an_almost_product_is_not_the_product.py'],
        'why': 'with no companion on file, the model dresses the table with look-alike plates and glasses from an imagined range',
    },
    {
        'name': 'other_products_are_redrawn_away',
        'file': 'app/creative.py',
        'find': '                    if best_v.get("other_products"):\n                        fixes.append("REMOVE every product that is not one of the "',
        'replace': '                    if False:  # SABOTAGE\n                        fixes.append("REMOVE every product that is not one of the "',
        'suites': ['test_an_almost_product_is_not_the_product.py'],
        'why': 'a frame with an invented second plate is redrawn without being told what was wrong, and comes back with the plate',
    },
    {
        'name': 'the_judge_is_asked_about_other_products',
        'file': 'app/creative.py',
        'find': '            "other_products": bool(data.get("other_products")), "why": ""}',
        'replace': '            "other_products": False, "why": ""}  # SABOTAGE',
        'suites': ['test_an_almost_product_is_not_the_product.py'],
        'why': 'the judge answers the question and its answer is thrown away, so a frame strewn with invented tableware ranks on product match alone',
    },
    {
    ...
    },
    {
        ...
        'file': 'app/creative.py',
        'find': '        for e, a in companions(tenant, ent, limit=CAST_INPUTS):\n            if a.id not in have and len(cast_rows) < CAST_INPUTS:\n                cast_rows.append(a)',
        'replace': '        for e, a in []:  # SABOTAGE\n            if a.id not in have and len(cast_rows) < CAST_INPUTS:\n                cast_rows.append(a)',
        'suites': ['test_an_almost_product_is_not_the_product.py'],
        'why': "the catalogue's companions never reach the table, so every tablescape is either bare or laid with invented pieces",
    },
    {
